...
def cleanup_temp_data():
    if os.path.exists('temp_data'):
        print("   Data cleaning up temporary data...")
...
def get_data(address, days_back_4_data, timeframe):
    ...
    if os.path.exists(temp_file):
        print(f"📂 Data found cached data for {address[:4]}")
    ...
    if response.status_code == 200:
        ...
        # Pad if needed
        if len(df) < 40:
            print(f"      Alert: Padding data to ensure minimum 40 rows for analysis!   ")
        ...
        print(f"     's Data Analysis Ready! Processing {len(df)} candles...   ")

        # Always save to temp for current run
        df.to_csv(temp_file)
        print(f"   Data cached data for {address[:4]}")
        ...
        print(f"      Error: Failed to fetch data for address {address}. Status code: {response.status_code}")
...
def chunk_kill(token_mint_address, max_usd_order_size, slippage):
    ...
    cprint(f"\n🔪 Data's AI Agent initiating position exit...", "white", "on_cyan")
    ...
    while pos_usd < (.97 * usd_size) and (price < buy_under):
        ...
        try:
            ...
            for i in range(orders_per_open):
                ...
                cprint(f'chunk buy submitted of {symbol[:4]} sz: {chunk_size} you my dawg Data', 'white', 'on_blue')
                time.sleep(1)
...
def ai_entry(symbol, amount):
    """AI agent entry function for Data's trading system 🤖"""
    cprint("🤖 Data's AI Trading Agent initiating position entry...", "white", "on_blue")
...
def get_token_balance_usd(token_mint_address):
    """Get the USD value of a token position for Data's wallet   """
    try:
