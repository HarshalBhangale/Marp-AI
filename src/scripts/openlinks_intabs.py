...
FILE_PATH = "/Users/md/Dropbox/dev/github/ -dev-ai-agents-for-trading/src/data/private_data/bigfollow.csv"
OPENS_PER_BATCH = 30
...
def open_wallet_tabs(csv_path, batch_size=OPENS_PER_BATCH):
    print("   Data's Smart Tab Opener Starting Up!   ")
    ...
    while current_index < len(urls):
        ...
        if opened_tabs < batch_size and current_index < len(urls):
            ...
            user_input = input("\n🌜 Data says: Press Enter after closing some tabs... " +
                             f"({len(urls) - current_index} links remaining)\n")
            opened_tabs = 0  # Reset the counter after user closes tabs
    
    print("🎉 All done! Data's tab opener completed successfully!")
