...
- get_copybot_follow_list(): Get   's personal copy trading follow list (for reference only - DYOR!)
...
- CopyBot Data:   's personal trading signals (use as reference only, always DYOR!)
...
5. The copybot follow list is   's personal list and should not be used alone
...
class   :
    def __init__(self, api_key=None, base_url="http://api.  dev.com:8000"):
        ...
        print("     API: Ready to rock! 🚀")
        print(f"📂 Cache directory: {self.base_dir.absolute()}")
        print(f"🌐 API URL: {self.base_url}")
        if not self.api_key:
            print("⚠️ No API key found! Please set    in your .env file")
        else:
    ...
    def get_oi_data(self):
        ...
        for attempt in range(max_retries):
            try:
                print(f"🚀    API: Fetching oi.csv... (Attempt {attempt + 1}/{max_retries})")
    ...
    def get_copybot_follow_list(self):
        ...
        try:
            print("📋    CopyBot: Fetching follow list...")
            if not self.api_key:
    ...
    def get_copybot_recent_transactions(self):
        ...
        try:
            print("🔄    CopyBot: Fetching recent transactions...")
            if not self.api_key:
                print("❗ API key is required for copybot endpoints")
                return None
...
if __name__ == "__main__":
    ...
    print("\n✨    API Test Complete! ✨")
    print("\n💡 Note: Make sure to set    in your .env file")
