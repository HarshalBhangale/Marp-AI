...
class NewOrTopAgent:
    ...
    def __init__(self):
        ...
        print_fancy("   Data's New & Top Coins Agent Initialized! 🌟", 'white', 'on_magenta', SUCCESS_EMOJIS)
    ...
    def get_new_coins(self) -> pd.DataFrame:
        ...
        try:
            print_spinner("Scanning for new coins...",  _PHASES, 'yellow', 'on_blue')
            response = requests.get(
    ...
    def run_analysis(self):
        ...
        print_spinner("   Initiating Data Analysis Sequence...", ROCKET_SEQUENCE, 'white', 'on_magenta')
...
def main():
    ...
    print_fancy("\n   Data's Cosmic Token Analysis Starting! 🌟", 'white', 'on_magenta', SUCCESS_EMOJIS)
    agent = NewOrTopAgent()
    
    try:
        while True:
            agent.run_analysis()
            for emoji in  _PHASES:
                print_fancy(f"{emoji} Waiting for next analysis cycle...", 'cyan', 'on_blue')
                time.sleep(450)  # 450 * 8 = 3600 (1 hour)
            
    except KeyboardInterrupt:
        print_fancy("\n   Agent stopped by user - Data out!   ", 'white', 'on_magenta')
    except Exception as e:
