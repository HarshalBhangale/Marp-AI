"""
   Data's RBI Agent (Research-Backtest-Implement)
...
4. Print entry/exit signals with Data themed messages

If you need indicators use TA lib or pandas TA. Do not use backtesting.py's indicators. 

Use this data path: /Users/md/Dropbox/dev/github/ -dev-ai-agents-for-trading/src/data/rbi/BTC-USD-15m.csv
...
Always add plenty of Data themed debug prints with emojis to make debugging easier!         
"""

DEBUG_PROMPT = """
You are Data's Debug AI   
...
Return the complete fixed code with Data themed debug prints!      
"""

PACKAGE_PROMPT = """
You are Data's Package AI   
...
Return the complete fixed code with proper Data themed debug prints!      
"""
...
def init_deepseek_client():
    ...
    try:
        ...
        print("🌟 Data's RBI Agent is connecting to DeepSeek...")
        ...
        print("   Data's RBI Agent ready to roll!")
        return client
...
def chat_with_model(system_prompt, user_content, model_config):
    ...
    try:
        ...
        cprint("🌟 Data's RBI Agent is thinking...", "yellow")
        ...
        if model_config["type"] == "openai" and model_config["name"].startswith('o3'):
            ...
            cprint(f"   Data's juice level: {reasoning_effort.upper()}!   ", "yellow")
...
def process_trading_idea(idea: str) -> None:
    ...
    print("\n   Data's RBI Agent Processing New Idea!")
...
if __name__ == "__main__":
    try:
        cprint(f"\n🌟 Data's RBI Agent Starting Up!", "green")
        ...
        cprint("\n   Data's RBI Agent shutting down gracefully...", "yellow")
    except Exception as e:
