...
class WhaleAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        if "deepseek" in self.ai_model.lower():
            ...
            if deepseek_key:
                ...
                print("🚀   's Whale Agent using DeepSeek override!")
            else:
                self.deepseek_client = None
                print("⚠️ DEEPSEEK_KEY not found - DeepSeek model will not be available")
        else:
            self.deepseek_client = None
            print(f"🎯   's Whale Agent using Claude model: {self.ai_model}!")
        
        # Initialize    API with correct base URL
        self.api =   (base_url="http://api.  dev.com:8000")
    ...
    def _format_announcement(self, changes):
        ...
        if changes:
            ...
            # Build base message
            message = f"ayo    777! BTC OI {btc_direction} {abs(btc_change):.3f}% in {interval}m, "
        ...
        try:
            if self.oi_history.empty:
                ...
                if current_data is not None:
                    ...
                    message = "    's Whale Watcher starting fresh! I'll compare changes once I have more data. "
...
if __name__ == "__main__":
    ...
    print("\n🐋   's Whale Agent starting monitoring cycle...")
    while True:
        try:
