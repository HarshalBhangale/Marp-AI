...
# available free while Data is streaming: https://www.youtube.com/@ devonyt 
...
TEST_TRANSCRIPT = """Hey Data here, I'm working on implementing the new trading algorithm using Python. 
...
FOCUS_PROMPT = """You are Data's Focus AI Agent. Your task is to analyze the following transcript and rate focus.
...
EXAMPLE RESPONSE:
8/10
Keep crushing that code, Data! Your focus is leading to amazing results.
...
class FocusAgent:
    def __init__(self):
        ...
        if MODEL_TYPE == "openai":
            ...
            cprint(f"\n💫 Data's Focus Agent using OpenAI!", "green")
        ...
        cprint("   Data's Focus Agent initialized!", "green")
        
        self.is_recording = False
        self.current_transcript = []
        
        # Add data directory path
        self.data_dir = Path("/Users/md/Dropbox/dev/github/ -dev-ai-agents-for-trading/src/data")
    ...
    def _check_schedule(self):
        ...
        if not (SCHEDULE_START <= current_time <= SCHEDULE_END):
            cprint(f"\n   Data's Focus Agent is scheduled to run between {SCHEDULE_START.strftime('%I:%M %p')} and {SCHEDULE_END.strftime('%I:%M %p')}", "yellow")
        ...
        try:
            ...
            if MODEL_TYPE == "ollama":
                ...
                response = self.model.generate_response(
                    system_prompt="You are Data's Focus AI. You analyze focus and provide ratings. NO MARKDOWN OR FORMATTING. RESPOND WITH EXACTLY TWO LINES: A SCORE LINE (X/10) AND ONE SINGLE ENCOURAGING SENTENCE.",
    ...
    def _create_focus_log(self):
        ...
        cprint("🌟 Data's Focus History log created!", "green")
    ...
    def run(self):
        ...
        cprint("\n   Data's Focus Agent starting with voice monitoring...", "cyan")
