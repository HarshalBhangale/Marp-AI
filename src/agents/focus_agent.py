...
# available free while    is streaming: https://www.youtube.com/@  devonyt 
...
TEST_TRANSCRIPT = """Hey    here, I'm working on implementing the new trading algorithm using Python. 
...
FOCUS_PROMPT = """You are   's Focus AI Agent. Your task is to analyze the following transcript and rate focus.
...
class FocusAgent:
    def __init__(self):
        ...
        if MODEL_TYPE == "openai":
            ...
            cprint(f"\n💫   's Focus Agent using OpenAI!", "green")
        ...
        cprint("🎯   's Focus Agent initialized!", "green")
    ...
    def _check_schedule(self):
        ...
        if not (SCHEDULE_START <= current_time <= SCHEDULE_END):
            cprint(f"\n    's Focus Agent is scheduled to run between {SCHEDULE_START.strftime('%I:%M %p')} and {SCHEDULE_END.strftime('%I:%M %p')}", "yellow")
        ...
        try:
            ...
            if MODEL_TYPE == "ollama":
                ...
                response = self.model.generate_response(
                    system_prompt="You are   's Focus AI. You analyze focus and provide ratings. NO MARKDOWN OR FORMATTING. RESPOND WITH EXACTLY TWO LINES: A SCORE LINE (X/10) AND ONE SINGLE ENCOURAGING SENTENCE.",
    ...
    def _create_focus_log(self):
        ...
        cprint("🌟   's Focus History log created!", "green")
    ...
    def run(self):
        ...
        cprint("\n🎯   's Focus Agent starting with voice monitoring...", "cyan")
