...
Need an API key? for a limited time, bootcamp members get free api keys for claude, openai, helius, birdeye & quant elite gets access to the    api.   
"""
...
class FundingAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        if "deepseek" in self.active_model.lower():
            ...
            if deepseek_key:
                ...
                cprint("🚀   's Funding Agent using DeepSeek override!", "green")
            else:
                self.deepseek_client = None
                cprint("⚠️ DEEPSEEK_KEY not found - DeepSeek model will not be available", "yellow")
        else:
            self.deepseek_client = None
            cprint(f"🎯   's Funding Agent using Claude model: {self.active_model}!", "green")
    ...
    def _format_announcement(self, opportunities):
        ...
        try:
            ...
            if messages:
                return "ayo    seven seven seven! " + " | ".join(messages) + "!"
            return None
