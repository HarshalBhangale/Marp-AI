#!/usr/bin/env python3
"""
Data's Backtest AI    – Dynamic Retest Strategy Backtesting & Optimization
---------------------------------------------------------------------
...
Charts are saved to the specified "charts" directory with Data themed file names.
...
class DynamicRetest(Strategy):
    ...
    def init(self):
        ...
        print("     Data: INIT complete – indicators (zone_top, zone_bottom, sma20) are online!   ")
    ...
    def next(self):
        ...
        # Debug print – current bar info with Data flair
        print(f"     Data: Processing bar {self.data.index[-1]} → Open: {current_open:.2f}, High: {current_high:.2f}, Low: {current_low:.2f}, Close: {current_close:.2f}")
        ...
        if self.position:
            print("     Data: Currently in a trade – holding position with lunar patience!")
            return
        ...
        if trend == 'up' and current_close > current_open:
            ...
            if curr_zone_bottom <= current_close <= curr_zone_top:
                ...
                if take_profit > current_close + (2 * recent_range):
                    print(f"   Data: TP target ({take_profit:.2f}) too far from current price. LONG trade skipped!")
                    return
                ...
                print(f"     Data: LONG signal detected! Entry = {current_close:.2f}, Stop Loss = {stop_loss:.2f}, Take Profit = {take_profit:.2f}, Size = {position_size}")
        ...
        if trend == 'down' and current_close < current_open:
            ...
            if curr_zone_bottom <= current_close <= curr_zone_top:
                ...
                recent_range = self.data.High[-self.consolidation_span:].max() - self.data.Low[-self.consolidation_span:].min()
                if take_profit < current_close - (2 * recent_range):
                    print(f"   Data: TP target ({take_profit:.2f}) too far from current price. SHORT trade skipped!")
                    return
                ...
                print(f"     Data: SHORT signal detected! Entry = {current_close:.2f}, Stop Loss = {stop_loss:.2f}, Take Profit = {take_profit:.2f}, Size = {position_size}")
                self.sell(size=position_size, sl=stop_loss, tp=take_profit)
                return

        # If no signal was generated, log it!
        print("  🔍 Data: No valid trade signal on this bar. Scanning the cosmos for the perfect setup…")

    def onexit(self, trade):
        # Called when a trade closes – print out Data themed messages!
...
print("     Data: Loading data from CSV…")
...
try:
    ...
    # ----------------------------
    print("\n     Data: Starting parameter optimization…")
