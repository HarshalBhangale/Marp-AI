Below is the complete backtesting.py implementation for the StochasticPhaseTrader strategy. Copy and run the code (after installing required packages such as backtesting, talib, pandas, etc.) and enjoy Data’s witty debug prints!       
...
• Debug prints (with plenty of Data themed emojis         ) are included at key stages (data loading, initialization, entry/DCA/exit signals).
