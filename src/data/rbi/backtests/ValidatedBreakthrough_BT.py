#!/usr/bin/env python3
"""
  's Backtest AI   — ValidatedBreakthrough Strategy Backtest Implementation
...
Remember:
  ...
  • Save all performance plots to the specified charts directory with    debug prints! 🚀✨
...
class ValidatedBreakthrough(Strategy):
    ...
    def init(self):
        ...
        print("  [  ] Indicators initialized: SMA period = {}, Lookback = {}".
              format(self.sma_period, self.lookback))
    
    def next(self):
        current_price = self.data.Close[-1]
        print("🚀 [  ] New bar processed – Current Price: {:.2f}".format(current_price))
        ...
        if not self.position:
            ...
            if current_price > self.sma[-1]:
                print("✨ [  ] Uptrend detected – Close {:.2f} > SMA {:.2f}".format(current_price, self.sma[-1]))
                ...
                if current_price <= self.recent_low[-1] * (1 + self.zone_buffer):
                    ...
                    if risk <= 0:
                        print("🚀 [  ] ERROR: Non‐positive risk detected. Skipping LONG trade!")
                    elif reward / risk < self.min_rr:
                        print("🚀 [  ] RR Ratio {:.2f} below minimum {:.2f}. Skipping LONG trade!".
                        ...
                        if position_size <= 0:
                            print("🚀 [  ] Calculated position size is 0. Trade skipped!")
                        else:
                            print("  [  ] Placing LONG order with size: {}".format(position_size))
                            self.buy(size=position_size, sl=stop_loss, tp=take_profit)
                else:
                    print("  [  ] Price not within demand zone threshold for LONG entry.")
            
            # -------------------
            # Check SHORT entries
            # -------------------
            elif current_price < self.sma[-1]:
                print("✨ [  ] Downtrend detected – Close {:.2f} < SMA {:.2f}".format(current_price, self.sma[-1]))
                ...
                if current_price >= self.recent_high[-1] * (1 - self.zone_buffer):
                    ...
                    if risk <= 0:
                        print("🚀 [  ] ERROR: Non‐positive risk detected. Skipping SHORT trade!")
                    elif reward / risk < self.min_rr:
                        print("🚀 [  ] RR Ratio {:.2f} below minimum {:.2f}. Skipping SHORT trade!".
                              format(reward / risk, self.min_rr))
                    else:
                        position_size = int(round((self.risk_pct * self.equity) / risk))
                        if position_size <= 0:
                            ...
                            print("  [  ] Placing SHORT order with size: {}".format(position_size))
                            self.sell(size=position_size, sl=stop_loss, tp=take_profit)
                else:
                    print("  [  ] Price not within supply zone threshold for SHORT entry.")
        else:
            print("  [  ] Active position exists – Monitoring trade exit conditions...")
...
if __name__ == '__main__':
    ...
    if unnamed_cols:
        print("  [  ] Dropping unnamed columns: {}".format(unnamed_cols))
    ...
    print("  [  ] Data cleaning complete – Columns: {}".format(list(data.columns)))
    ...
    print("🚀 [  ] Starting initial backtest with cash: ${}".format(initial_cash))
    ...
    print("🚀 [  ] Saving initial performance chart to: {}".format(initial_chart_file))
    ...
    print("  [  ] Optimized Backtest Stats:")
    print(optimized_bt)
    print("  [  ] Optimized Strategy parameters used: ")
    ...
    print("🚀 [  ] Saving optimized performance chart to: {}".format(optimized_chart_file))
    optimized_bt.plot(filename=optimized_chart_file, open_browser=False)
    
    print("✨  [  ] Backtest execution complete – Keep reaching for the   ! 🚀")
