...
print("   Data: Data loaded and cleaned! Total rows:", len(df))
...
class TimeframeTrendDivergence(Strategy):
    ...
    def init(self):
        # Data: Build aggregated series for multi-timeframe analysis.
        # Data is 15-minute candles. We now create weekly, daily, 4hour, 1hour resampled bars.
        print("       Data: Initializing aggregated timeframes…")
        ...
        print("   Data: Aggregated timeframes ready.")
    ...
    def next(self):
        ...
        if not self.position and self.pending_signal is None:
            ...
            if bullish_weekly and bullish_daily:
                print("   Data: Weekly & Daily are bullish! (🌞)")
                ...
                print(f"   Data: 4H consolidation check: current_range = {current_range:.2f}, median_range = {median_range:.2f}, consolidation = {consolidation}")
                ...
                if consolidation and (bullish_h1 or bearish_h1):
                    ...
                    print(f"   Data: 1H indicates a {direction} trend. Setting pending signal: {self.pending_signal} (👀)")
        ...
        if self.pending_signal is not None and not self.position:
            ...
            if self.pending_signal == 'pending_long' and current_price > h1_bar['High']:
                print(f"   Data: LONG entry trigger! Price broke above 1H High {h1_bar['High']:.2f}   ")
                ...
                print(f"   Data: SHORT entry trigger! Price broke below 1H Low {h1_bar['Low']:.2f}   ")
        ...
        if self.position:
            print(f"   Data: In trade: {self.position} with entry price {self.position.entry_price:.2f}")
    ...
    def enter_trade(self, trade_dir, entry_price, h1_bar):
        ...
        if trade_dir == 'long':
            ...
            if risk_per_unit <= 0:
                print("   Data: Risk per unit <= 0. Trade skipped. 😱")
                return
        ...
        print(f"   Data: Entering {direction_text} trade at {entry_price:.2f} with stop loss {stop_loss:.2f} and take profit {take_profit:.2f}.")
        print(f"   Data: Equity = {equity:.2f}, Risk amount = {risk_amount:.2f}, Risk per unit = {risk_per_unit:.2f}, Calculated size = {position_size} units.")
        ...
        # Data debug message after entering
        print(f"   Data: {direction_text} trade executed!     ")
...
if __name__ == '__main__':
    ...
    print("       Data: Running initial backtest...")
    stats = bt.run()
    print("\n   Data: Initial Backtest Results:")
    print(stats)
    ...
    print(f"   Data: Saving initial chart to: {chart_file}")
    ...
    print("\n   Data: Optimized Backtest Results:")
    print(optimized_stats)
    print("\n   Data: Optimized Strategy parameters:")
    ...
    chart_file = os.path.join(chart_dir, f"{strategy_name}_chart.html")
    print(f"   Data: Saving optimized chart to: {chart_file}")
    bt.plot(filename=chart_file, open_browser=False)

    print("   Data: Backtesting and optimization complete!     ")
...
• Plenty of debug prints (with Data emojis) help track the flow of signals.
...
Happy backtesting with Data’s Backtest AI!       