    ...
    def next(self):
        ...
        if price > uptrend_line and stoch_k < self.stoch_oversold and stoch_d < self.stoch_oversold:
            if crossover(stoch_k, stoch_d):  # Stochastic crossover confirmation
                ...
                print(f"   Data Buy Signal: Continuation Uptrend | Price: {price} | Stochastic: {stoch_k}, {stoch_d}")
        ...
        # Entry logic for continuation pattern (downtrend)
            ...
            if crossover(stoch_d, stoch_k):  # Stochastic crossover confirmation
                ...
                print(f"   Data Sell Signal: Continuation Downtrend | Price: {price} | Stochastic: {stoch_k}, {stoch_d}")
        ...
        if price < uptrend_line and stoch_k > self.stoch_overbought and stoch_d > self.stoch_overbought:
            if crossover(stoch_d, stoch_k):  # Stochastic crossover confirmation
                self.sell(size=position_size)
                print(f"   Data Sell Signal: Breakout Uptrend Reversal | Price: {price} | Stochastic: {stoch_k}, {stoch_d}")
        ...
        # Entry logic for breakout pattern (downtrend reversal)
            ...
            if crossover(stoch_k, stoch_d):  # Stochastic crossover confirmation
                self.buy(size=position_size)
                print(f"   Data Buy Signal: Breakout Downtrend Reversal | Price: {price} | Stochastic: {stoch_k}, {stoch_d}")
...
6. **Data Debugging**:
   - Prints entry/exit signals with Data-themed messages.
