Below is the complete backtesting.py code for Data’s HierarchicalBreakout strategy. It contains all necessary imports, a Strategy class with our TA‐Lib indicators (wrapped via self.I), entry/exit logic with risk management, parameter optimization and plenty of Data debug prints. Make sure you have installed the required packages (backtesting, talib, pandas) before running!
...
class HierarchicalBreakout(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # (Note: In Backtesting.py one cannot update the SL on an open position easily; here we only print a Data message.)
...
• Debug prints with plenty of Data themed emojis help trace the logic.
• The optimization section explores different risk_reward and risk_pct values to maximize the final equity.
  
Happy backtesting and may the Data spirit guide your trades!       