...
def collect_token_data(token, days_back=DAYSBACK_4_DATA, timeframe=DATA_TIMEFRAME):
    ...
    cprint(f"\n🤖   's AI Agent fetching data for {token}...", "white", "on_blue")
    ...
    try:
        ...
        if data is None or data.empty:
            cprint(f"❌   's AI Agent couldn't fetch data for {token}", "white", "on_red")
            return None
            
        cprint(f"📊   's AI Agent processed {len(data)} candles for analysis", "white", "on_blue")
        ...
        cprint(f"💾   's AI Agent cached data for {token[:4]}", "white", "on_green")
        
        return data
        
    except Exception as e:
        cprint(f"❌   's AI Agent encountered an error: {str(e)}", "white", "on_red")
        return None
...
def collect_all_tokens():
    ...
    cprint("\n🔍   's AI Agent starting market data collection...", "white", "on_blue")
    ...
    cprint("\n✨   's AI Agent completed market data collection!", "white", "on_green")
...
if __name__ == "__main__":
    try:
        ...
        print(f"❌ Error: {str(e)}")
        print("🔧    suggests checking the logs and trying again!") 